...
度量空间（详见`【模】（2）线性空间`）是常用的拓扑空间，其上开集的定义为

$$
\tau=\{S\subseteq X|\forall x\in S,\exists\epsilon>0,\{y\in X|d(x,y)<\epsilon\}\subseteq S\}
$$
...
1. **最值定理**：闭区间（区域）上的一元（多元）连续实值函数必然存在最大值和最小值（统称最值）。
2. **介值定理**：对于闭区间（区域）$D$ 上的一元（多元）连续实值函数 $f$，若 $a,b\in D$，    
   $\qquad\qquad$ 则 $f(x)$ 在 $D$ 上可取 $f(a)$ 和 $f(b)$ 之间任何值。    
   结合最值定理和介值定理，闭区间（区域）$D$ 上的一元（多元）连续实值函数 $f$ 的值域
   $$\{f(x)|x\in D\}=\{y\in\R|m\le y\le M\}$$
    其中 $m,M$ 分别为 $f$ 在 $D$ 上的最大值和最小值。  
    对于 $\R$ 上一元函数，该定理等价于
	> **零点存在定理**：若函数 $f:[a,b]\rightarrow\R$ 连续，且 $f(a)f(b)<0$，则 $f(x)$ 在 $(a,b)$ 内存在零点。  
	> 等价性证明：零点存在定理显然对于介值定理是必要条件，现证其充分性，即  
	> $\qquad\qquad[a,b]$ 上的连续实值函数 $f(x)$ 可取 $f(a)$ 和 $f(b)$ 之间（包含 $f(a),f(b)$）的任何值。  
	> $\qquad\qquad$ 对于 $f(a)$ 和 $f(b)$ 之间（包含 $f(a),f(b)$）的任意实数 $\eta$，  
...
设 $n=b^m$，等式两边同除以 $a^m$ 得

$$
\dfrac{T(b^m)}{a^m}=\dfrac{T(b^{m-1})}{a^{m-1}}+\dfrac{f(b^m)}{a^m}
$$
$$
\Rightarrow \dfrac{T(b^m)}{a^m}=\Theta\left(\sum_{k=1}^m\dfrac{f(b^k)}{a^k}\right)
$$
