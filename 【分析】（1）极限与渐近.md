...
1. > 极限的唯一性*：对于拓扑空间 $(Y,\tau)$，若对于其中任意两个不同的点 $u,v\in X$，  
   ...
   > 以它们为值域的映射不满足极限的唯一性。 
2. > 复合映射的极限：对于映射 $f:X\rightarrow Y$ 和 $g:Y\rightarrow Z$，若 $\displaystyle\lim_{\mathcal{B}_X}f(x)=a,\lim_{\mathcal{B}_Y}g(y)=b$ 存在，且
   > $$\forall B_Y\in\mathcal{B}_Y,\exist B_X\in\mathcal{B}_X,\{f(x)|x\in\mathcal{B}_X\}\subseteq B_Y$$
   > $\qquad\qquad\qquad$ 则 $\displaystyle\lim_{B_X}(g\circ f)(x)=b$ 。  
   > 复合映射极限的关键在于
