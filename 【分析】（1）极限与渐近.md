...
2. $\displaystyle\lim_{x\rightarrow\infty}\left(1+\dfrac{1}{x}\right)^x=e$（$e$ 的定义式）
   > 存在性证明：首先证明数列极限 $\displaystyle e=\lim_{n\rightarrow+\infty}\left(1+\dfrac{1}{n}\right)^n$ 存在。
   ...
   > > 又因为 $\dfrac{n^{\underline{n}}}{n!n^n}>0$ 恒成立，因此 $a_n<a_{n+1}$，即数列 $a$ 单调递增。又因为
   > > $$\forall k\in[0,n]\cap\N,2^{k-1}\le k!,1-\dfrac{k}{n}<1$$
   > > 故
   > >$$a_n<\sum_{k=0}^n\dfrac{1}{k!}\le\sum_{k=0}^n2^{1-k}=\dfrac{2^{-n}-2^{}}{2^{-1}-1}=4-2^{1-n}<4$$
   > > 因此单调递增数列 $\{a_n\}$ 存在上界 $4$。故 $\displaystyle\lim_{n\rightarrow+\infty}a_n$ 存在。  
   >
   > 接下来，用数列极限证明对应的函数极限存在。$x\ge 1$ 时，设 $n=\left\lfloor x\right\rfloor$，则
...
闭区间（区域）上的一元（多元）连续函数满足以下两条关键性质：

1. **最大最小值定理**：闭区间（区域）上的一元（多元）连续函数必然存在最大值和最小值（统称最值）。
2. **介值定理**：设闭区间（区域）$D$ 上的一元（多元）连续函数 $f$ 的最大值为 $M$，最小值为 $m$，则
   $$\{f(x)|x\in D\}=[m,M]$$
    对于一元函数，该定理等价于
	> **零点存在定理**：若 $f(x)$ 在 $[a,b]$ 上连续，且 $f(a)f(b)<0$，则 $f(x)$ 在 $(a,b)$ 内存在零点。
