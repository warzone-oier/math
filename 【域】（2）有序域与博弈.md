...
其中 $L,R$ 均为博弈组成的集合，分别表示若第一个回合属于左方/右方，左方/右方可选择的决策。  
为保证游戏一定会在有限步后结束（即无环），根据正则公理（详见`【引言】集合、映射与基本逻辑`），**全体博弈不构成集合**。    
即，只能从已构造的博弈中选取元素组成 $L,R$，进而构造一个新博弈 $\{L|R\}$。
...
在此基础上继续构造，就可以构造出所有的博弈，乃至决策集合为无限集的博弈（称为无限博弈）。  
...
>   
> $g+(-g)\equiv 0$ 等价于 $g+(-g)$ 后手必胜。以上证明实际上给出了后手的必胜策略：  
> 模仿先手在对应的反博弈上进行操作。
